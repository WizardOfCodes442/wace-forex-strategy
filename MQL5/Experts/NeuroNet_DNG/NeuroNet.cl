         ...
         switch(stop - k)
           {
            ...
            case 2:
               inp =
                  (float4)(matrix_i[shift_in + k], matrix_i[shift_in + k + 1], 1, 0);
               weight = (float4)(matrix_w[shift + k], matrix_w[shift + k + 1],
                                 matrix_w[shift + k + 2], 0);
               break;
            case 3:
   ...
   for(int t = 0; t < total; t++)
     {
      if(shift != window_in && (shift + t * window_in) >= inputs)
         break;
      grad += matrix_g[t * window_out + shift_out] *
              (shift == window_in ? 1 : matrix_i[shift + t * step]);
     }
         ...
         for(int t = 0; t < total; t++)
           {
            if(i != window_in && (i + t * window_in) >= inputs)
               break;
            grad += (float4)(matrix_g[t * window_out + out],
                             matrix_g[t * window_out + out + 1],
                             matrix_g[t * window_out + out + 2],
                             matrix_g[t * window_out + out + 3]) *
                    (i == window_in ? 1 : matrix_i[i + t * step]);
           }
         float4 mt = clamp(b1 * (float4)(matrix_m[shift_w],
                                         matrix_m[shift_w + window_in + 1],
                                         matrix_m[shift_w + 2 * (window_in + 1)],
                                         matrix_m[shift_w + 3 * (window_in + 1)]) +
                           (1 - b1) * grad,
                           -1.0e5f, 1.0e5f);
         float4 vt = clamp(b2 * (float4)(matrix_v[shift_w],
                                         matrix_v[shift_w + window_in + 1],
                                         matrix_v[shift_w + 2 * (window_in + 1)],
                                         matrix_v[shift_w + 3 * (window_in + 1)]) +
                           (1 - b2) * pow(grad, 2),
                           1.0e-6f, 1.0e6f);
         float4 delta = l * mt / sqrt(vt);
         float4 weight =
            clamp((float4)(matrix_w[shift_w], matrix_w[shift_w + (window_in + 1)],
         ...
         for(int t = 0; t < total; t++)
           {
            if(i != window_in && (i + t * window_in) >= inputs)
               break;
            grad += matrix_g[t * window_out + out] *
                    (i == window_in ? 1 : matrix_i[i + t * step]);
           }
         float mt =
            clamp(b1 * matrix_m[shift_w] + (1 - b1) * grad, -1.0e5f, 1.0e5f);
         float vt = clamp(b2 * matrix_v[shift_w] + (1 - b2) * pow(grad, 2),
                          1.0e-6f, 1.0e6f);
...
//---
   float mean =
      (options[shift] * ((float)batch - 1.0f) + inputs[n]) / ((float)batch);
   ...
   for(uint i = 0; i < total; i += 4)
     {
      if(total - i > 4)
         sum +=
            dot((float4)(output[i], output[i + 1], output[i + 2], output[i + 3]),
   ...
   for(uint i = 0; i < inputs_size; i += 4)
     {
      if(total - i > 4)
         sum +=
            dot((float4)(inputs[i], inputs[i + 1], inputs[i + 2], inputs[i + 3]),
                (float4)(weights[shift + i], weights[shift + i + 1],
                         weights[shift + i + 2], weights[shift + i + 3]));
      else
         for(uint k = i; k < total; k++)
...
__kernel void LSTM_ConcatenatedGradient(__global float *gradient,
                                        ...
                                        __global float *concatenated)
  {
   ...
   concatenated_gradient[id + 3 * total] =
      memory_gradient * concatenated[id + total]; // new content
   concatenated_gradient[id + total] =
      memory_gradient * concatenated[id + 3 * total]; // input gate
   concatenated_gradient[id] =
      memory_gradient * memory[id + total]; // forgat gate
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
   ...
   for(int v = q_id; v < kunits; v += qunits)
     {
      ...
      for(int g = 0; g < qunits; g++)
         grad += gradient[shift_g + g * dimension] *
...
//|                                                                  |
//+------------------------------------------------------------------+
__kernel void DeActivation(__global float *inputs, __global float *inputs_gr,
                           __global float *output_gr, int activation)
  {
   int n = get_global_id(0);
//---
   float inp = inputs[n];
...
//|                                                                  |
//+------------------------------------------------------------------+
__kernel void MatMult(__global float *matr1,
                      __global float *matr2,
...
//---
   int shift1 = row * dimension;
//---
   float res = 0;
   for(int i = 0; i < dimension; i++)
      res += matr1[shift1 + i] * matr2[col + i * cols];
//---
   result[row * cols + col] = res;
  }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
